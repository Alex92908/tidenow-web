...
export async function fetch(): Promise<NewsItem[]> {
  const res = await myFetch("https://openai.com/blog/rss.xml", {
  ...
  for (const match of entries) {
    ...
    const category = block.match(/<category><!\[CDATA\[([\s\S]*?)\]\]><\/category>/)?.[1]?.trim()
      ?? block.match(/<category>([\s\S]*?)<\/category>/)?.[1]?.trim() ?? ""
    if (title && link) {
      items.push({ id: `openai-${i}`, title, url: link, extra: category || undefined })
    }
    i++
    if (i >= 25) break
  }
