...
FUNNEL_PROMPT = """你是克制的"漏斗选篮"研究员。从以下业绩预增候选中挑 8-15 只组成观察篮。
偏好：政策主线（新质生产力：AI算力/半导体/机器人/高端制造/创新药等）里的**卖铲子位置**
（零部件/材料/设备/耗材供应商），回避：纯概念壳、题材蹭热点、整机豪赌、已被爆炒的票。
只依据给到的信息判断行业与位置，不臆测其它。

候选（代码 名称 | 净利预增幅% | 60日涨幅%）：
{cands}

只返回 JSON：{{"picks": [{{"code": "600183", "tag": "卖铲子-覆铜板", "why": "一句话"}}]}}
"""
